...
def backup_file(file_path):
    ...
    backup_dir = os.path.join(os.path.dirname(file_path), '..', 'backups', 'uuid_migration')
...
def migrate_file(file_path, delimiter=';', dry_run=False):
    ...
    lines_needing_uuid = [i for i, line in enumerate(lines) if not has_uuid(line, delimiter)]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Adiciona UUIDs a registros existentes')
    parser.add_argument('--dry-run', action='store_true',
                       help='Simula a migração sem fazer alterações')
    parser.add_argument('--path', default='data/txt',
